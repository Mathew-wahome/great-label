...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      <button 
          onClick={onStartOver}
          className="absolute top-4 left-4 z-30 flex items-center justify-center text-center bg-black/60 border border-gray-700/80 text-gray-200 font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-black hover:border-gray-600 active:scale-95 text-sm backdrop-blur-sm"
      >
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {displayImageUrl ? (
          ...
          />
            ...
            <div className="w-[400px] h-[600px] bg-gray-900 border border-gray-800 rounded-lg flex flex-col items-center justify-center">
              <Spinner />
              <p className="text-md font-serif text-gray-400 mt-4">Loading Model...</p>
            </div>
        )}
        ...
        <AnimatePresence>
          {isLoading && (
              <motion.div
                  className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-20 rounded-lg"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
              >
      ...
      {displayImageUrl && !isLoading && (
        ...
        >
          <AnimatePresence>
              {isPoseMenuOpen && (
                  <motion.div
                      ...
                      className="absolute bottom-full mb-3 w-64 bg-black/80 backdrop-blur-lg rounded-xl p-2 border border-gray-700/80"
                  >
                      <div className="grid grid-cols-2 gap-2">
                          {poseInstructions.map((pose, index) => (
                              <button
                                  ...
                                  className="w-full text-left text-sm font-medium text-gray-200 p-2 rounded-md hover:bg-gray-700/70 disabled:opacity-50 disabled:bg-gray-700/70 disabled:font-bold disabled:cursor-not-allowed"
                              >
                                  {pose}
                              </button>
                          ))}
          ...
          <div className="flex items-center justify-center gap-2 bg-black/60 backdrop-blur-md rounded-full p-2 border border-gray-700/50">
            <button 
              onClick={handlePreviousPose}
              aria-label="Previous pose"
              className="p-2 rounded-full hover:bg-black/80 active:scale-90 transition-all disabled:opacity-50"
              disabled={isLoading}
            >
              <ChevronLeftIcon className="w-5 h-5 text-gray-200" />
            </button>
            <span className="text-sm font-semibold text-gray-200 w-48 text-center truncate" title={poseInstructions[currentPoseIndex]}>
